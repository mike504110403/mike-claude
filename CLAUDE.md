...
| 前端   | **`/local-stack` 起地端棧**（地端＝Mac 本機或 PC 延伸機 `ai-pc-wsl`，由 skill 依 `pc-reach` 自動選，2026-09-16 起；「API 位址指向地端」同此定義） | typecheck ＋ **Mike 地端手測**（review chain 並行站，見 /review-chain；先驗 API 位址指向地端）；/auto-e2e 僅 on-demand |
