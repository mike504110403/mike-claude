...
| 前端   | **`/local-stack` 起地端棧** | typecheck ＋ **/auto-e2e** 瀏覽器實測證據（依 /vue-dev）；Mike 手動實測在 **/ship**（push 前批次）                        |
| 前後端 | **同上，聯測走地端**        | 兩者皆備，且合併回 dev 前 **/auto-e2e** 走跨端關鍵流程；合併後依 /feature-flow 階段四重驗；Mike 手動實測在 **/ship** 批次 |
