...
| 前端   | **`/local-stack` 起地端棧** | typecheck ＋ 瀏覽器實測**交 Mike 手動**；Mike 不在或明示代跑時改走 **/auto-e2e**（依 /vue-dev）                                         |
| 前後端 | **同上，聯測走地端**        | 兩者皆備，且合併回 dev 前**聯測**：/local-stack 起地端棧、環境就緒後**交 Mike 手動**走關鍵流程；Mike 不在或明示代跑時改走 **/auto-e2e** |
