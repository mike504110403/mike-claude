...
// [Mike 2026-09-08] 全域 shim 守衛：cwd 沒有 graft 圖就零副作用退出（不建 ~/graft、不掃家目錄、不注入升級提示）。
if (!fs.existsSync(path.join(dir, 'graft', '.graph', 'wiring.json'))) process.exit(0);
const BAKED = "/Users/mike/.nvm/versions/node/v20.20.0/lib/node_modules/@nanonets/graft/dist/claude";
