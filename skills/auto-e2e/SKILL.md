...
1. **環境**：/local-stack 起棧；API 指地端判準唯一依據 /vue-dev「瀏覽器實測」0-1 步。`BASE_URL` 一律顯式傳入，spec 內不得寫死任何位址（防指到正式線）。測試帳號取自該專案 `.claude/localstack.json`。開跑前在 cmux 內把 `BASE_URL` 開成內嵌瀏覽器 tab（開法唯一依據全域 CLAUDE.md「Browser 自動化工具鏈」末條：加 tab、不開新 pane）——headless 測試本身照跑，tab 是給 Mike 同步看被測站與事後手測用；/local-stack 已開過就不重開。
