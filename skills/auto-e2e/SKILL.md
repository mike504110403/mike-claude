...
1. **環境**：/local-stack 起棧；API 指地端判準唯一依據 /vue-dev「瀏覽器實測」0-1 步。`BASE_URL` 一律顯式傳入，spec 內不得寫死任何位址（防指到正式線）。測試帳號取自該專案 `.claude/localstack.json`。
...
- Chrome MCP 僅剩用途＝/bug 互動式診斷（逐步看 network/console）；動用時瀏覽器 agent 同時僅 1。
