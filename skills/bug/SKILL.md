...
2. **診斷段**：依 /diagnosing-bugs — 先重現、再縮小範圍、定位根因，重現成功才進入修改。
   ...
   - 互動式瀏覽器診斷（逐步看 network/console）用 **playwright-cli**（`goto`／`snapshot`／`console`／`requests`，具名 session）；頁面結構試探不出才切 Playwright MCP、效能類（trace／lighthouse／heap）才用 chrome-devtools MCP——選擇依 /browser-tools。
