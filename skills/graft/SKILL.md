---
name: graft
description: 全域 graft 探路 skill——只在當前 repo 已有 graft/（跑過 graft build）時生效。任何要找程式碼位置、追呼叫鏈、算改動影響面、看檔案 API 面的任務，先用 graft 再 grep／讀檔。
---

# graft（全域版，2026-09-08 自 @nanonets/graft 0.15.0 的 skill 模板移植）

## 本機慣例（Mike／同事 Ray 共用，覆蓋下方通用說明）

- **先確認 `graft/` 存在**（`ls graft/.graph/wiring.json`）；沒有就照該 repo README「Code graph（graft）」段建：`graft build --lsp --no-gitignore --no-ignore .`。目前只有 Go 後端 repo 建圖（lottery-platform、gold-price）；Vue repo 未驗、不用。
- **查詢一律加 `--no-refresh`**：自動 refresh 會把 gopls 的 LSP 邊丟掉；改檔後圖要更新就重跑 `graft build --lsp`。
- 每個 git worktree 是獨立目錄，**各自 build 一次**（cache 共用、秒級）。
- `ask` 只吃英文 lexical，中文查不到。
...
- 接線全在 `~/.claude`：hook shim `helpers/graft-hooks-global.cjs`（**故意不用原檔名 `graft-hooks.cjs`**——graft 啟動時看到 `<cwd>/.claude/helpers/graft-hooks.cjs` 就會把 cwd 當已接線 repo 重寫整套設定，包含覆蓋本檔與 statusline）、`settings.json` 五個 hook（SessionStart／UserPromptSubmit／PostToolUse×2／Stop）、`~/.claude.json` 的 `mcpServers.graft`。**任何專案 repo 內不放 graft 的 `.claude/` 接線**（Mike 2026-09-08 裁示）。
- **post-edit 只標 dirty＋印 blast radius，真正重建圖的是 Stop hook**；上游 `sync-run.js` 跑裸 `graft build .` 會把 gopls 邊丟掉（實測 15135→11464 邊），所以 Stop 指令帶 `GRAFT_TEST_SYNC_RUN=~/.claude/helpers/graft-sync-lsp.mjs`，改跑 `build --lsp --no-gitignore --no-ignore`（實測 13s 內恢復 15135 邊）。shim 開頭另有守衛：cwd 沒有 `graft/.graph/wiring.json` 直接退出，大腦 session（cwd=~）與 Vue repo 零副作用。
- 升級 graft 版本後：手動重跑一次沙箱 `graft init` 取新模板比對本檔與 shim，不要在專案或家目錄跑 `graft init`。
