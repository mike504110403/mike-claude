...
**在 cmux 內時（`$CMUX_BUNDLED_CLI_PATH` 存在）先把前端位址開成內嵌瀏覽器 pane**，作為 Mike 手測入口：

```
"$CMUX_BUNDLED_CLI_PATH" browser open <前端URL>
```

（工具定位與退回規則見全域 CLAUDE.md「Browser 自動化工具鏈」末條；前端 URL 取自宣告檔的 `frontend` 區塊。）
