...
1. **宣告**：第一句「走 /quick，因為…；改動面：後端／前端／前後端」。驗證義務照全域「改動面 × 驗證義務」矩陣，不在此重抄。
2. 依 /feature-flow 階段一開需求 worktree（quick 也開——主 checkout 恆 dev 不直改），大腦在需求 worktree 裡**直接改**（不寫 brief、不派工）。
